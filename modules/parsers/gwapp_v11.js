...
export function parseGwAppV11(lines, skippableWargearMap = {}) {
    ...
    const line1Match = line1.match(/^(.*?)\s*\((\d+)\s*points\)$/i);
    if (line1Match) {
    ...
    if (nonEmptyLines[2]) {
        ...
        const line3Match = line3.match(/^(.*?)\s*\((\d+)\s*points\)$/i);
        if (line3Match) {
    ...
    let headerOffset = 3;
    if (nonEmptyLines[3] && !nonEmptyLines[3].toLowerCase().startsWith('force dispositions:')) {
        const line4 = nonEmptyLines[3].trim();
        const line4Match = line4.match(/^(.*?)\s*\((\d+)\s*Detachment\s*Points\)$/i);
        if (line4Match) {
            const detStr = line4Match[1].trim();
            result.metadata.detachmentPoints = parseInt(line4Match[2], 10) || 0;
            // Split by " and "
            result.metadata.detachments = detStr.split(/\s+and\s+/i).map(d => d.trim()).filter(Boolean);
        } else {
    ...
    if (nonEmptyLines[headerOffset] && nonEmptyLines[headerOffset].toLowerCase().startsWith('force dispositions:')) {
        const line5 = nonEmptyLines[headerOffset].trim();
        const dispStr = line5.replace(/^Force Dispositions:\s*/i, '').trim();
    ...
    const parseUnitBlock = (unitName, unitPoints, blockLines) => {
        ...
        const findNodes = (nodes) => {
            nodes.forEach(node => {
                ...
                if (contentLower.startsWith('attached as:')) {
                    attachedAsNode = node;
                }
                if (node.indent === 2) {
                    elements.push(node);
        ...
        if (attachedAsNode) {
            unit.attachedAs = attachedAsNode.content.replace(/^Attached as:\s*/i, '').trim();
            unit.role = unit.attachedAs.toLowerCase().includes('leader') ? 'Leader' : 'Bodyguard';
        }
        ...
        elements.forEach(node => {
            ...
            if (contentLower === 'warlord') {
                ...
                const enhName = content.replace(/^Enhancement:\s*/i, '').trim();
    ...
    while (i < cleanLines.length) {
        ...
        if (line.toLowerCase().startsWith('force dispositions:') || line.toLowerCase().startsWith('attached units') || line.toLowerCase().startsWith('characters') || line.toLowerCase().startsWith('dedicated transports') || line.toLowerCase().startsWith('other datasheets')) {
            break;
        }
        i++;
    }
    if (i < cleanLines.length && cleanLines[i].trim().toLowerCase().startsWith('force dispositions:')) {
        i++; // skip force dispositions line
    }

    const sectionHeaders = ['attached units', 'characters', 'dedicated transports', 'other datasheets'];
    ...
    while (i < cleanLines.length) {
        ...
        if (sectionHeaders.includes(trimmedLower)) {
            currentSection = trimmed; // Keep casing
        ...
        if (currentSection.toLowerCase() === 'attached units' && /^Attached Unit \d+$/i.test(trimmed)) {
        ...
        const unitMatch = trimmed.match(/^(.*?)\s*\((\d+)\s*(?:pts|points)\)$/i);
        if (unitMatch) {
            ...
            while (i < cleanLines.length) {
                ...
                const nextTrimmedLower = nextTrimmed.toLowerCase();
                if (sectionHeaders.includes(nextTrimmedLower) ||
                    (currentSection.toLowerCase() === 'attached units' && /^Attached Unit \d+$/i.test(nextTrimmed)) ||
                    nextTrimmed.match(/^(.*?)\s*\((\d+)\s*(?:pts|points)\)$/i) ||
                    nextTrimmed.toLowerCase().startsWith('exported with app version:')) {
                    break;
                }
            ...
            if (currentSection.toLowerCase() === 'attached units' && currentAttachedGroup) {
